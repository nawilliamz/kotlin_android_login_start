...
class MainFragment : Fragment() {
    ...
    //What we're doing here in onActivityResult is listening for the sign-in request code to come back
    //Once we know that we're processing the result of our login,
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == SIGN_IN_RESULT_CODE) {
            ...
            Log.i(TAG, "Successfully signed in user ${FirebaseAuth.getInstance().currentUser?.displayName}!") } else {
        ...
        viewModel.authenticationState.observe(viewLifecycleOwner, Observer { authenticationState ->
            when (authenticationState) {
                LoginViewModel.AuthenticationState.AUTHENTICATED -> {
                    ...
                    //updates the text that is displayed on the homescreenj by inccluding the user's name
                    ...
                    //If there is no login user, we want to make sure we start in a fresh state where the
                    //auth button displays Login and the fun fact displayed doesn't include the user's name
    ...
    //Pre-appends the factoid with the first name of the currently logged in user.
    ...
    private fun launchSignInFlow() {
        ...
        //What we're doing here is telling Firebase UI how we want to let the user sign in.
        ...
        startActivityForResult(
            AuthUI.getInstance()
                ...
                .build(),
            //We're listening to the result of the sign in process with a sign-in request code defined at the
            //beginning of the file. Since we started the sign-in process with the sign-in request code,
            //we can also listen to the result of the sign-in process by filtering for when this request code is
            //passed back in OnActivityResult().
